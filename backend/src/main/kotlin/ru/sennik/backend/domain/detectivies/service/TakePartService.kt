...
class TakePartService(
   ...
   fun getAllTakePartDetectives(crimeId: Long) = repository.findByCrimeId(crimeId)

   fun getTakePartDetective(crimeId: Long, detectiveId: Long) =
      repository.findByCrimeIdAndDetectiveId(crimeId, detectiveId)
         ?: throw NotFoundException("Детектив с id=$detectiveId не участвовал в раскрытии преступления с id=$crimeId")

   @Transactional
   fun createTakePart(crimeId: Long, detectiveId: Long): TakePart {
      val detective = detectiveService.getDetectiveById(detectiveId)
      crimeId.also { crimeService.getCrimeById(crimeId) }
      return repository.findByCrimeIdAndDetectiveId(crimeId, detectiveId)
         ?.let {
            throw AlreadyExistException(
               "Детектив с id=$detectiveId уже принимает участие в раскрытии преступления с id=$crimeId"
            )
         }
         ?: repository.save(TakePart(detective = detective, crimeId = crimeId))
   }
